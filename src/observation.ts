...
export async function capture(page: Page, options: { scope?: string; maxElements: number; maxTexts: number }): Promise<Captured> {
  ...
  const data: Snapshot = {
    id: randomUUID(), url: publicURL(rawURL), title: await page.title(), elements: [], texts: [],
  ...
  try {
    for (const [frameIndex, frame] of page.frames().entries()) {
      ...
      const observe = new Function('roots', `${source()}; return JevDOM.observe(${JSON.stringify(frameOptions)}, roots);`) as (roots?: Element[]) => ReturnType<typeof DOM.observe>;
      const result = await frame.evaluateHandle(observe, roots);
      owned.push(result);
      const observed = await result.evaluate(r => ({ elements: r.elements, texts: r.texts, truncatedElements: r.truncatedElements, truncatedTexts: r.truncatedTexts }));
