...
Natural-language instructions, observed page URL/title, visible semantic text, element names, context, options, href evidence, action history and available input binding names may be sent to the configured decision endpoint. Hosted Jev is the default; `baseURL` / `JEV_BASE_URL` can redirect the same System One wire format to another trusted endpoint. Custom endpoints may log or retain data independently of this package. They no longer implicitly inherit cloud environment credentials: supply `apiKey` explicitly or set `JEV_ENDPOINT_API_KEY` for an authenticated custom endpoint. The HTTPS hosted origin uses the usual cloud-key environment variables. The upstream SDK `TYPESAFE_BASE_URL` does not implicitly redirect this library. Explicit input values in `values` are withheld by default. `run(instruction, { semanticInputs: { "/country": 0.8 } })` explicitly permits only the named input paths to be disclosed when semantic native-option matching is needed. Other named input values are not added to that request. Input echoed into page text, labels, option values, link query strings, or instructions can still contain secrets. This is not a general redaction filter. Quoted input is already part of the instruction and is not secret from the provider.
...
## Shared boundary details

See [shared browser-boundary contracts](docs/browser-boundaries.md) for observed scope through delayed/compound operations, Page/core dialog and chooser ownership, current checkpoint adoption, escaped evidence paths, and preservation of primary errors when continuation preparation fails. These are runtime consistency checks, not browser/network sandbox guarantees.
