...
Goals may also report `complete / ui-readback` after selecting a fresh result record and comparing its observed values locally. The result distinguishes caller checks, inferred UI evidence, readback fields and unobserved fields. A matching UI is not proof of database durability or exactly-once execution. Known string echoes are redacted in goal requests/results without changing protocol IDs; this remains a best-effort filter, not a general data-loss-prevention boundary. Normal confirm/alert responses use current dialog identity and both authorization hooks; a model judgment is not an authorization service. Confidence is not authorization, calibration of end-to-end correctness, or a test oracle.
