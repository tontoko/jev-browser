...
test('semantic Locator: cancellation during a wait does not start provider work',async t=>{const {core,page}=await setup(t,'<main></main>',forbidden);const signal=AbortSignal.timeout(50);await assert.rejects(core.assertSemantic({actual:{locator:page.locator('#absent')},expected:'A'},{signal,timeoutMs:5000}));});
