...
test('unsupported nested transforms are refused explicitly',async t=>{
  const {core}=await fixture(t,'<p>A</p>');
  await assert.rejects(core.extract('値',z.object({person:z.object({name:z.string()}).transform(value=>value)})),{code:'UNSUPPORTED_SCHEMA'});
});
