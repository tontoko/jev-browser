...
export async function checkInstalledScreen(pkg, directory, env) {
  ...
  const run = args => new Promise((resolve,reject) => {
    ...
    const timer=setTimeout(()=>{child.kill('SIGKILL');reject(new Error('Installed screen process timed out.'));},60000);
    child.once('error',error=>{clearTimeout(timer);reject(error);});
    child.once('close',code=>{clearTimeout(timer);try{resolve({code,...JSON.parse(stdout)});}catch{reject(new Error('Installed screen process did not return JSON: '+stderr));}});
  });
  const cli = args => run([join(pkg,'dist','cli.js'),...args]);
  let client;
  try {
    // Isolate the installed Playwright module from the checkout's test imports.
    const sdk=await run(['--input-type=module','-e',`
      import assert from 'node:assert/strict';
      import { JevBrowser } from '@tontoko/jev-browser';
      const core=await JevBrowser.launch({screenOnly:true});
      try {
        await core.page.goto(process.argv[1]);
        ...
        assert.equal(r.frames[0].mimeType,'image/png');
        console.log(JSON.stringify({installedScreenSDK:true}));
      } finally { await core.close(); }
    `,url]);
    assert.equal(sdk.code,0);assert.equal(sdk.installedScreenSDK,true);assert.equal(saves,1);
    try {
    ...
    await client?.close();site.closeAllConnections();await new Promise(resolve=>site.close(resolve));
  }
}
