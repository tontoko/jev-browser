...
| `run(instruction, {values?, maxSteps?, until?, ...options}?)` | `{status, reason, steps}` |
...
Inputs can be explicit named `values`, or quoted strings copied verbatim from the instruction. Explicit values take precedence; they are not generated by Jev. Current quote syntax is ASCII double quotes, Japanese corner quotes and curly double quotes. Use named values for complex escaping or secrets.
