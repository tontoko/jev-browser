...
The runtime observes the current page, asks independent input-to-control questions together, checks collisions and actual form ownership before writing, and uses native Playwright primitives serially. It retains earlier wizard-step coverage, verifies current field values, re-observes changed select options and repairs a field reset by another input. Unchanged state is waited on locally rather than repeatedly sent to Jev.

A primary binding does not grant permission to ignore other supplied values. Missing, conflicting or unsupported inputs stop before commit. Required confirmation fields may reuse an existing supplied value only after a separate grounded decision; unrelated missing data is not invented. Exact native select labels/values are resolved locally. The goal does not expose all hidden input values to solve arbitrary synonyms.
...
The current automatic path is DOM-observable, predominantly native-form creation and explicit caller assertions. Custom widget semantics, hidden-value synonym matching, arbitrary multi-record creation, visual-only/Canvas interaction and business workflows requiring multiple independent commits are not universally automated. A hinted menu (`aria-haspopup`) offers hover; not every native command is automatically a goal candidate. SDK callers retain the genuine Playwright Page; CLI/MCP retain the documented native tools for unsupported steps. No arbitrary model-generated code, site-specific workflow DSL or mandatory second planning model is added.
